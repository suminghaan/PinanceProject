...
<body>
  ...
  <div id="wrapper">
	   	 ...
	   	 <div class="container-fluid">
	   	 	...
	   	 	<div class="row" style="margin-top:50px">
	   	 		<div class="col-12">
	   	 			<div class="card" style="border: 1px solid #4A55A2; width: 98%; margin-left: auto; margin-right: auto; border-radius: 10px;">
	   	 				<div class="card-body">
	   	 					...
	   	 					<!-- 컨텐츠 영역-------------------------------- -->
	   	 					<h4 class="header-title" style="font-size:1.5rem">전자결재 수정</h4><br>
	   	 					
	   	 					<form id="edocModify" action="${contextPath}/edoc/modifyEdoc.do" method="post" enctype="multipart/form-data">
	                        ...
	                        <option selected>${edoc.sampleDto.sampleCode}</option>
	                        ...
	                        <option value="${edoc.docCode}" selected>${edoc.sampleDto.sampleTitle}</option>
                    		</select>
                    	</td>   	 								
	   	 								<td>사용여부</td>   	 								
	   	 								<td>
	   	 									<select name="sampleStatus" class="form-control" disabled>
	                        ...
	                        <option value="2">미사용</option>
	   	 							...
	   	 							<tr>
	   	 								<td>양식명</td>
	   	 								<td>
	   	 									<input type="text" class="form-control" id="sampleTitle" value="${edoc.sampleDto.sampleTitle}" readonly>
	   	 								</td>
	   	 								<td>양식설명</td>
	   	 								<td>
	   	 									<input type="text" class="form-control" id="sampleDesc" value="${edoc.sampleDto.sampleDesc}" readonly>
	   	 								</td>
	   	 							</tr>
	   	 							<tr>
	   	 								<td>비밀등급</td>
	   	 								<td>
	   	 									<select id="secCode" name="secCode" class="form-select">
	                        ...
	                        <option value="A">A등급</option>
	   	 								...
	   	 								<td>보존연한</td>
	   	 								<td>
	   	 									<select id="preservePeriod" name="preservePeriod" class="form-select">
                      ...
                      <div class="col-md-10">
                          <input type="text" id="docTitle" class="form-control" name="docTitle" value="${edoc.docTitle}" required>
                      </div>
                  </div>
	   	 						
	   	 						<br><br>
	   	 						
	   	 						<textarea name="docContent" id="editorTxt" row="20" cols="20" style="width:100%; min-height: 500px;">${edoc.docContent}</textarea>
	   	 						<br><br>
	   	 						<div class="mb-2 row">
                      <label class="col-md-2 col-form-label" for="example-fileinput">첨부 파일</label>
                      <div class="col-md-10">
                        <input name="uploadFile" type="file" class="form-control" id="example-fileinput" multiple>
                      </div>
                  ...
                  <br><br>
	   	 						...
	   	 						<c:choose>
	   	 							<c:when test="${ edoc.tempSave eq 'Y' }">
	   	 								<button type="button" style="margin-right: 20px" class="btn btn-primary" onclick="modifyEdoc();">등록하기</button>	   	 						
	   	 							...
	   	 							<c:otherwise>
		   	 							<button type="button" style="margin-right: 20px" class="btn btn-primary" onclick="modifyEdoc();">수정하기</button>
      ...
      $(document).ready(function() {
        ...
        $("#secCode").val("${edoc.secCode}").prop("selected", true);
        $("#preservePeriod").val("${edoc.preservePeriod}").prop("selected", true);
        ...
        $(".origin_del").on("click", function(){
		 			...
		 			document.getElementById("edocModify").append(inputEl);
      ...
      function notificationEdoc(edocType, refType){
				// 결재자 알람 보내기 위해 사용
			  var apUser = []; // 결재자 정보를 담을 배열
		    ...
		    $('.apLine tbody tr').each(function() {
		      ...
		      apUser.push(user); // 배열에 추가
			  	...
			  	if(apUser.length > 0){
			  		console.log(apUser);
			  		notification(edocType, apUser[0].id);
			  	}
				// 참조자 알람 보내기 위해 사용
		    var refUsers = []; // 참조자 정보를 담을 배열
		    ...
		    $('.refLine tbody tr').each(function(){
		    	...
		    	refUsers.push(refUser);	    	
				 ...
				 for(let i = 0; i < refUsers.length; i++){
					 notification(refType, refUsers[i].id);
				 }
			 	}
  	 	}
    	// 기안수정
    	function modifyEdoc(){
    		 notificationEdoc("modifyEdoc", "refModifyEdoc");
    	...
    	function deleteEdoc(no){
  			notificationEdoc("deleteEdoc", "refDeleteEdoc");

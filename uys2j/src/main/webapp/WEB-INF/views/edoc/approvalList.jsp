                  ...
                  <div class="page-title-box page-title-box-alt">
                      ...
                      <div class="page-title-right">
                          <ol class="breadcrumb m-0">
                              ...
                              <li class="breadcrumb-item"><a href="javascript: void(0);">결재대기함</a></li>
                          </ol>
                      </div>
                  </div>
              </div>
          ...
          <div class="row">
              <div class="col-12">
                  <div class="card" style="border: 1px solid #4A55A2; width: 98%; margin-left: auto; margin-right: auto; border-radius: 10px;">
                      <div class="card-body">
                          <h4 class="header-title">결재대기함</h4>
                          <p class="sub-header">
                           결재 요청 목록
                          </p>
                              ...
                              <table id="demo-foo-filtering" class="table table-bordered toggle-circle mb-0" data-page-size="10">
                                  ...
                                  <tbody>
	                                  <c:forEach var="edoc" items="${list}">                                
	                                      ...
	                                      <td>${edoc.docNo}</td>
	                                      <td>${edoc.docTitle}</td>
	                                      <td>${edoc.defaultDto.regId}</td>
	                                      <td>${edoc.defaultDto.regTime}</td>
	                                      <td>
	                                      	<c:choose>
	                                      		<c:when test="${ edoc.status eq 'W' }">
	                                      			<span class="badge label-table bg-success">진행중</span>
	                                      				<c:if test="${ not empty edoc.apUserid }">
	                                      					<span class="badge label-table bg-success">결재</span>
	                                      				</c:if>
	                                      				<c:if test="${ not empty edoc.refUserid }">
	                                      					<span class="badge label-table bg-secondary text-light">참조</span>
	                                      				</c:if>
	                                      		</c:when>
	                                      		<c:otherwise>
	                                      			<span class="badge label-table bg-secondary text-light">대기</span>
	                                      			<c:if test="${ not empty edoc.apUserid }">
                                      					<span class="badge label-table bg-success">결재</span>
                                      				</c:if>
                                      				<c:if test="${ not empty edoc.refUserid }">
